...
**Priority**: P1
**Status**: ✅ Done — `app/websocket/page_orchestrator.py` implemented as an async coroutine `run_page(session_id, page_number, beat, page_history, emit, story_planner, character_bible_svc, image_svc, tts_svc, media_svc, session_store) -> None`. Full 7-step pipeline: (1) emits `page_generating`, persists `Page(status="pending")` via `SessionStore`; (2) calls `story_planner.expand_page` → `(text, narration_script)`, emits `page_text_ready`, updates `Page.status = text_ready`; (3) fetches `CharacterBible` via `session_store.get_character_bible`, calls `character_bible_svc.build_image_prompt`; (4) launches image (`image_svc.generate → media_svc.store_illustration → signed URL → page_image_ready`) and TTS (`tts_svc.synthesize → media_svc.store_narration → signed URL → page_audio_ready`) **in parallel** via `asyncio.gather(return_exceptions=True)` — each arm catches all exceptions internally and emits `page_asset_failed` instead; (5) calls `character_bible_svc.set_reference_image(gcs_uri)` on page-1 image success; (6) persists final `Page` (with `illustration_failed`/`audio_failed` flags and `status = complete`); (7) emits `page_complete` — **always fires as the final event**. `CharacterBibleService` extended with `build_image_prompt(bible, page_text, page_number) -> ImagePrompt` (constructs a rich Imagen prompt with protagonist description, style, and content-policy negatives; includes `reference_image_gcs_uri` as reference on pages ≥ 2) and `async set_reference_image(session_id, gcs_uri)` (persists protagonist ref URI via `update_character_bible_field`). 11 mock-based tests covering TEST-P01 (event order, text content, always-fires, 4 unique types, page-number propagation, flags on success) and TEST-P02 (image fail, audio fail, both fail, reference set on p1 success, reference not set on p1 failure). 726 total passing (pre-existing integration tests excluded). Ruff clean. Verified: all 11 tests pass (`pytest tests/test_t025_page_orchestrator.py -v` — 11 passed, 0 failed).
**Files**:
- `voice-story-agent/backend/app/websocket/page_orchestrator.py`
- `voice-story-agent/backend/app/services/character_bible_service.py` (add build_image_prompt, set_reference_image)
- `voice-story-agent/backend/tests/test_t025_page_orchestrator.py`
...
**Priority**: P1
**Status**: ✅ Done — `app/routers/pages.py` extended with `POST /sessions/{id}/pages/generate` endpoint (202 Accepted): validates session exists (404), status=generating (409), no open steering window (409 if current page still pending), story arc ready; enqueues `run_page` as a FastAPI `BackgroundTask`; returns `{session_id, page_number, status:"generating"}`. `app/websocket/story_ws.py` extended with `_page_generation_loop` async coroutine and wired into `_turn_loop`: after `_route_user_turn` completes setup (session status becomes `generating`), spawns `_page_generation_loop` as a background `asyncio.Task`. Loop: for page 1–5, fetches session/arc, calls `run_page` (emits all page events via WS), then emits `steering_window_open` + sleeps `_STEERING_WINDOW_SECONDS` (10 s) + emits `steering_window_closed` between pages; finally emits `story_complete` and updates `Session.status = complete`. `app/dependencies.py` extended with singletons for `StoryPlannerService`, `CharacterBibleService`, `ImageGenerationService`, `TTSService`, `MediaPersistenceService`. `story_websocket` FastAPI endpoint updated to inject and forward all 5 new services to `_turn_loop`. 15 mock-based tests covering REST endpoint (202, 404, 409 variants, page_number=current+1) and loop (story_complete as final event, status=complete, 5 page_complete events, session_id propagation, 4 steering window pairs, graceful abort on session-not-found and short arc). 740 total passing (1 pre-existing flaky safety test excluded). Ruff clean.
**Files**:
- `voice-story-agent/backend/app/routers/pages.py` (extend)
- `voice-story-agent/backend/app/websocket/story_ws.py` (extend)
- `voice-story-agent/backend/app/dependencies.py` (add service singletons)
- `voice-story-agent/backend/tests/test_t026_page_generate_and_loop.py`
...
**Priority**: P2
**Status**: ✅ Done — `app/websocket/story_ws.py` extended with two new client message types and `_PageLoopState` dataclass. `_PageLoopState` (frozen dataclass): `interrupt_event: asyncio.Event`, `steering_turn_queue: asyncio.Queue`, `in_steering_window: bool` — shared between the page-generation loop background task and the WebSocket dispatch loop. `_page_generation_loop` updated to accept `page_loop_state: _PageLoopState | None` — per page: clears `interrupt_event`, wraps `run_page` as a task, races it against `interrupt_event.wait()` via `asyncio.wait(FIRST_COMPLETED)`; if interrupt wins, page task is cancelled and the loop proceeds directly to the steering window; the steering window passes `steering_turn_queue` to `SteeringHandler.run_steering_window`. **`interrupt` message**: if `not in_steering_window`, sets `page_loop_state.interrupt_event` causing page task cancellation and immediate steering window entry; ignored if already in steering window. **`voice_feedback` message**: if `in_steering_window`, puts a synthetic `VoiceTurn(role="user", transcript=raw_transcript, is_final=True)` onto `steering_turn_queue` for the active `SteeringHandler`; if outside steering window, runs safety evaluation and routes as a normal transcript turn (same safety + route pipeline as `transcript_input`). `_turn_loop` and `story_websocket` updated to create and thread `page_loop_state` through. 14 mock-based tests covering all 3 spec "Done when" criteria: interrupt → `steering_window_open`, `voice_feedback` tone_change → `voice_command_received`+`voice_command_applied`, unsafe `voice_feedback` → safety flow + no arc update — plus `_PageLoopState` mechanics, interrupt race condition, command_type propagation, pacing arc update, and combined interrupt+voice_feedback flow. 792 total passing (30 integration tests deselected). Ruff clean.
**Files**:
- `voice-story-agent/backend/app/websocket/story_ws.py` (extend)
- `voice-story-agent/backend/tests/test_t031_interrupt_voice_feedback.py`
...
**Priority**: P1
**Status**: ✅ Done — `voice-story-agent/frontend/src/hooks/useStoryState.ts` implemented as a `"use client"` React hook accepting a `WsClient | null`. Exports `PageState { pageNumber, text, imageUrl, audioUrl, illustrationFailed, audioFailed, status }`, `Caption { role, text, turnId, isFinal }`, and `HydrateSession` types. Subscribes to `page_text_ready` (sets `text`, `status="text_ready"`), `page_image_ready` (sets `imageUrl`, advances status), `page_audio_ready` (sets `audioUrl`, advances status), `page_asset_failed` (sets `illustrationFailed` or `audioFailed` depending on `asset_type`), `page_complete` (sets `status="complete"`, persists `illustration_failed`/`audio_failed`), `transcript` (appends to `captions` for `is_final=true` only — partial transcripts ignored), `steering_window_open` (sets `steeringWindowOpen=true`, `steeringWindowPage`), `steering_window_closed` (sets `steeringWindowOpen=false`). Each update produces a new immutable `PageState` object via functional `setPages` with spread. `hydrate(session)` iterates `session.pages` and merges `text`, `illustration_failed`, `audio_failed`, `status` into existing page entries without overwriting WS-received `imageUrl`/`audioUrl`; no-op if `pages` is null or empty. `reset()` restores all state to empty/false/null. Subscriptions only applied when `client` changes (tracked via `useRef`). No `any` in public API; `tsc --noEmit` exits 0. 27 Jest tests in `src/hooks/__tests__/useStoryState.test.tsx` covering all done-when criteria: `page_image_ready` sets `imageUrl`, creates new page entry, doesn't affect other pages; `page_asset_failed` illustration sets `illustrationFailed`, narration sets `audioFailed`; `hydrate` populates pages, null/empty pages are no-ops, merges with existing WS state; `page_text_ready` sets text+status, `page_audio_ready` sets audioUrl, `page_complete` sets status+flags; final transcript added to captions, partial transcript ignored, multiple transcripts accumulate; steering window open/close; multiple events same page accumulate; `reset()` clears pages/captions/steeringWindow; null client is safe. 72 total passing (27 wsClient T-034 + 18 useVoiceSession T-035 + 27 new). `tsc --noEmit` exits 0.
**Files**:
- `voice-story-agent/frontend/src/hooks/useStoryState.ts` (new)
- `voice-story-agent/frontend/src/hooks/__tests__/useStoryState.test.tsx` (new)
...
**Priority**: P1
**Status**: ✅ Done — `voice-story-agent/frontend/src/app/story/page.tsx` replaced stub with `"use client"` `StoryAppPage` component. Wires together: `useVoiceSession` for session/audio lifecycle; a `WsClient` created in `useEffect` when `voice.sessionId` becomes non-null (torn down when it returns to null); `useStoryState(wsClient)` for per-page/caption/steering state. The WS client subscribes to: `page_generating` → `setIsGenerating(true)`, `page_complete` → `setIsGenerating(false)`, `story_complete` → `setStoryComplete(true)`, `safety_rewrite` → `setSafetyRewrite(evt)` + clears `safetyAccepted`, `safety_accepted` → `setSafetyAccepted(evt)`, `connected` → reconnect recovery guard (second+ call fetches `GET /sessions/{id}` and calls `story.hydrate(data)`, first call sets `hasConnectedOnceRef.current=true`). Renders: error banner (`data-testid="session-error-banner"`, `role="alert"`) when `voice.error` is set; `data-testid="story-book-wrapper"` div containing `<StoryBook pages isGenerating storyComplete steeringWindowOpen totalPages={5}>`; `<CaptionBar captions safetyRewrite safetyAccepted className="pb-28">`; floating `<VoiceButton>` with `onInterrupt` (sends `interrupt` message) and `onFeedback` (calls `startSession`/`stopSession`). `npm run build` produces `/story` route at 5.99 kB, `tsc --noEmit` exits 0. 18 Jest tests in `src/app/__tests__/StoryAppPage.test.tsx` (mocking `WsClient`, `useVoiceSession`, `useStoryState`) covering all done-when criteria: renders story-app-page/story-book-wrapper/VoiceButton/CaptionBar/StoryBook without runtime errors; error banner shown/hidden; VoiceButton aria-pressed from isListening; WsClient created+connected when sessionId set; WsClient not created when sessionId null; second connected event triggers `GET /sessions/{sessionId}` + hydrate; first connected event does NOT hydrate; fetch errors swallowed; page_generating → HoldAnimation visible; page_complete → HoldAnimation hidden; story_complete → closing card; safety_rewrite → amber card; safety_accepted → green card replaces amber. 218 total passing. `tsc --noEmit` exits 0. `npm run build` exits 0.
**Files**:
- `voice-story-agent/frontend/src/app/story/page.tsx` (replace stub)
- `voice-story-agent/frontend/src/app/__tests__/StoryAppPage.test.tsx` (new)
...
**Priority**: P1
**Status**: ✅ Done — `voice-story-agent/backend/Dockerfile` finalised: added `HEALTHCHECK --interval=30s --timeout=5s --start-period=30s --retries=3` that calls `GET /health` via `urllib.request` inside the container (no extra curl dependency needed); `EXPOSE 8080`, `PYTHONUNBUFFERED=1`, non-root `appuser`, multi-stage builder+runtime, `CMD uvicorn app.main:app --host 0.0.0.0 --port ${PORT}` all retained. `voice-story-agent/infra/cloud-run-deploy.sh` written (executable, `chmod +x`): `set -euo pipefail`; argument validation with usage message; prerequisites check (gcloud + docker + authed account); Step 1 `docker build … --tag gcr.io/{PROJECT}/{SERVICE}:{TAG}`; Step 2 `gcloud auth configure-docker && docker push`; Step 3 `gcloud run deploy --image … --region … --platform=managed --allow-unauthenticated --service-account voice-story-agent-sa@… --set-env-vars` forwarding all 12 config.py vars (`GCP_PROJECT_ID`, `GCP_REGION`, `GCS_BUCKET_NAME`, `FIRESTORE_DATABASE`, `GEMINI_PRO_MODEL`, `GEMINI_FLASH_MODEL`, `IMAGEN_MODEL`, `GEMINI_LIVE_MODEL`, `TTS_VOICE_NAME`, `TTS_LANGUAGE_CODE`, `ADK_AGENT_NAME`, `CORS_ORIGINS`) plus Cloud Run sizing flags; Step 4 `gcloud run services describe --format="value(status.url)"` + prints health-check URL and WebSocket endpoint. `voice-story-agent/.github/workflows/deploy-backend.yml` written: triggers on `push` to `main` (paths filter: backend/**, infra/cloud-run-deploy.sh, workflow itself) and `workflow_dispatch` (image_tag + region inputs); jobs: `deploy` on `ubuntu-latest` with `id-token: write`; steps — checkout, `google-github-actions/auth@v2` (SA key or WIF), `google-github-actions/setup-gcloud@v2`, `configure-docker`, determine image tag (input or git SHA), docker build (SHA tag + latest), docker push both tags, `gcloud run deploy` with all config.py secrets/env-vars, `gcloud run services describe` URL + GitHub step summary; smoke test `curl /health` with 5 retries; `notify-failure` job on failure. 60 pytest tests in `tests/test_t042_cloud_run_deploy_config.py` validating: Dockerfile (exists, multi-stage, python:3.11-slim, appuser, EXPOSE 8080, PYTHONUNBUFFERED, uvicorn CMD, HEALTHCHECK, /health in healthcheck, PORT env-var); deploy script (exists, executable, strict mode, 4 steps, --allow-unauthenticated, --service-account, usage message, shebang, all 11 env vars, --region, --platform=managed, gcr.io URI, --port=8080); workflow (exists, push+dispatch triggers, auth step, build/push/deploy steps, health smoke test, GCP_PROJECT_ID/GCP_SA_KEY secrets, all 8 model env vars, ubuntu-latest, backend path filter, self path filter). 922 passing (60 new + 862 pre-existing; 2 pre-existing flaky safety integration tests excluded). `./infra/cloud-run-deploy.sh` deploys the service on a live GCP project when run with valid credentials.
**Files**:
- `voice-story-agent/backend/Dockerfile` (finalise — add HEALTHCHECK)
- `voice-story-agent/infra/cloud-run-deploy.sh` (new)
- `voice-story-agent/.github/workflows/deploy-backend.yml` (new)
- `voice-story-agent/backend/tests/test_t042_cloud_run_deploy_config.py` (new)
...
**Priority**: P2
**Status**: ✅ Done — `voice-story-agent/backend/app/logging_config.py` created: `StructuredJsonFormatter` (emits single-line JSON per record with `severity`, `message`, `timestamp`, `logger`, plus any `extra={}` fields); `_to_severity()` mapping Python levels → Cloud Logging severity strings; `configure_logging()` bootstrapper — installs JSON handler when `K_SERVICE` env var is set (Cloud Run) or `LOG_FORMAT=json`, falls back to plain text formatter in local dev; `LatencyLogger` context manager — wraps any `await` call, emits `event_type`, `duration_ms`, `success` and forwarded kwargs on exit. `app/main.py` updated: replaces `logging.basicConfig` with `configure_logging()` call on startup; startup logs include `event_type="startup"`. `app/services/safety_service.py` instrumented: `evaluate()` emits `event_type="safety_decision"` (with `session_id`, `safe`, `category`) — NEVER logs `raw_input` or utterance text; classifier error emits `event_type="safety_classifier_error"` (only `session_id`, `error_type`); parse error emits `event_type="safety_parse_error"`. `app/websocket/page_orchestrator.py` instrumented: `run_page()` emits `event_type="page_generation_started"` (session_id, page_number); Gemini `expand_page` call timed → `event_type="gemini_call_latency"` (duration_ms, operation); asset success → `event_type="page_asset_ready"` (asset_type); asset failure → `event_type="page_asset_failed"` (asset_type, error_type); page complete → `event_type="page_generation_complete"` (duration_ms, illustration_failed, audio_failed). `app/websocket/story_ws.py` instrumented: WS accept → `event_type="ws_connect"` (session_id, session_status); WS token/session reject → `event_type="ws_rejected"` (reason); `finally` cleanup → `event_type="ws_disconnect"`; `session_start` applied → `event_type="voice_command_applied"` (command=session_start); user turn received → `event_type="voice_command_received"`; `interrupt` applied → `event_type="voice_command_applied"` (command=interrupt); `voice_feedback` received → `event_type="voice_command_received"`; session status→generating → `event_type="session_status_changed"`. 61 pytest tests in `tests/test_t044_cloud_logging.py` covering: JSON formatter (valid JSON, severity mapping for all 5 levels, message/timestamp/logger fields, session_id/event_type/duration_ms/extra forwarding, raw_input not leaked); _to_severity mapping; configure_logging (JSON vs plain formatter, K_SERVICE env, LOG_FORMAT=json env); LatencyLogger (event_type, duration_ms, extra kwargs, success flag); SafetyService structured logs live assertions (safe=True logs safety_decision, unsafe logs category NOT raw utterance, classifier error logs event_type not utterance); module structure (importable, callable); page_orchestrator instrumentation static analysis (page_generation_started, page_generation_complete, gemini_call_latency, page_asset_ready, page_asset_failed, duration_ms, time import, session_id, page_number in extras); story_ws instrumentation static analysis (ws_connect, ws_disconnect, ws_rejected, voice_command_received, voice_command_applied, session_status_changed, session_id context); safety_service instrumentation static analysis (safety_decision, safety_classifier_error, category logged, raw_input not in extra dicts, main.py uses configure_logging, logging_config.py exists). 994 total passing (61 new + 933 previous; 2 pre-existing flaky safety tests excluded).
**Files**:
- `voice-story-agent/backend/app/logging_config.py` (new)
- `voice-story-agent/backend/app/main.py` (modified)
- `voice-story-agent/backend/app/services/safety_service.py` (modified)
